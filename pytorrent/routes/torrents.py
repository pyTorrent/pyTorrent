...
def _profile_transfer_payload(source_profile: dict, data: dict, *, require_hashes: bool = True) -> dict:
    """Validate a cross-profile transfer while keeping metadata-only paths independent from target storage."""
    # Note: A metadata-only transfer preserves each torrent's live d.directory; target_path is relevant only when data files are physically moved.
    ...
    roots = _target_profile_allowed_roots(target_profile, user_id) if requested_move_data else []
    default_target_path = (
        roots[0] if roots else _clean_remote_transfer_path(rtorrent.default_download_path(target_profile))
    ) if requested_move_data else ""
    target_path = _clean_remote_transfer_path(requested_target_path or default_target_path) if requested_move_data else ""
    inside_allowed_root = bool(requested_move_data and roots and any(_path_inside_root(target_path, root) for root in roots))
    requested_outside_roots = bool(requested_move_data and requested_target_path and not inside_allowed_root)
    ...
    if requested_outside_roots:
        # Note: Outside-root validation is needed only for a physical data move; metadata-only transfers do not use target_path for torrent data.
    ...
    # Note: If a requested data move is downgraded, the unused target path must not block the resulting metadata-only transfer.
    target_outside_roots = bool(move_data and requested_outside_roots)
    return {
        "hashes": hashes,
...
def profile_transfer_validate():
    ...
    try:
        ...
        # Note: Metadata-only validation skips remote disk-stat work because no data bytes are written to target_path.
        disk = (
            rtorrent.disk_usage_for_paths(target_profile, [payload["target_path"]], mode="selected", selected_path=payload["target_path"])
        ...
        return ok({
            ...
            "preserve_source_path": bool(payload.get("preserve_source_path")),
