...
def dry_run(profile: dict | None = None, user_id: int | None = None, data: dict[str, Any] | None = None) -> dict[str, Any]:
    """Return a read-only Smart Queue execution plan without mixing simulation code into live queue logic."""
    # Note: Smart Queue Simulation lives in its own module; this wrapper preserves the public service API.
    from . import smart_queue_simulation

    return smart_queue_simulation.dry_run(profile=profile, user_id=user_id, data=data)
